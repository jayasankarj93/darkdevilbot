...
import darkdevilbot.modules.sql.notes_sql as sql
from darkdevilbot import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from darkdevilbot.__main__ import DATA_IMPORT
from darkdevilbot.modules.helper_funcs.chat_status import user_admin
from darkdevilbot.modules.helper_funcs.alternate import typing_action

# from darkdevilbot.modules.rules import get_rules
import darkdevilbot.modules.sql.rules_sql as rulessql

# from darkdevilbot.modules.sql import warns_sql as warnssql
import darkdevilbot.modules.sql.blacklist_sql as blacklistsql
from darkdevilbot.modules.sql import disable_sql as disabledsql

# from darkdevilbot.modules.sql import cust_filters_sql as filtersql
# import darkdevilbot.modules.sql.welcome_sql as welcsql
import darkdevilbot.modules.sql.locks_sql as locksql
from darkdevilbot.modules.connection import connected
...
def export_data(update, context):
    ...
    f = open("darkdevilbot{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("darkdevilbot{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `darkdevilbot-Backup` was specially made for notes."
        .format(chat.title, chat_id, tgl),
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN,
    )
    os.remove("darkdevilbot{}.backup".format(chat_id))  # Cleaning file
