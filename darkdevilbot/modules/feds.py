...
import darkdevilbot.modules.sql.feds_sql as sql
from darkdevilbot import (GBAN_LOGS, LOGGER, OWNER_ID, SUDO_USERS, TIGER_USERS,
                          WHITELIST_USERS, dispatcher)
from darkdevilbot.modules.disable import DisableAbleCommandHandler
from darkdevilbot.modules.helper_funcs.alternate import send_message
from darkdevilbot.modules.helper_funcs.chat_status import is_user_admin
from darkdevilbot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from darkdevilbot.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "darkdevilbot_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="darkdevilbot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "darkdevilbot_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="darkdevilbot_fbanned_users.csv",
